...
export async function loadCompanies() {
  try {
    const list = await api.companies.list();
    companiesCache = Array.isArray(list) ? list : [];
    emitCompanies();
    return companiesCache;
  } catch (err) {
    // 🆕 Если PRIVATE без прав на список компаний — спокойно возвращаем что есть
    console.error("Failed to load companies:", err);
    return companiesCache;
  }
}
...
/**
 * 🆕 Возвращает выбранную компанию.
 * Если компания не найдена в кеше (например у PRIVATE кеш пустой),
 * возвращает «стаб»-объект с id, чтобы формы не ругались на отсутствие компании.
 */
export function getSelectedCompany() {
  const id = getSelectedCompanyId();
  if (!id) return null;
  const found = companiesCache.find((c) => c.id === id);
  if (found) return found;
  // Стаб для случаев когда компания не в кеше (PRIVATE)
  return { id, name: "", logo: "", phone: "", address: "" };
}
