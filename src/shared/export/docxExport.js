...
// Читает исходные размеры картинки (PNG/JPEG) из ArrayBuffer.
// Нужно, чтобы держать фикс. высоту и пропорциональную ширину.
function getImageDims(arrayBuffer) {
  try {
    const bytes = new Uint8Array(arrayBuffer);
    // PNG: сигнатура 89 50 4E 47, ширина/высота — 4-байтовые big-endian на offset 16 и 20
    if (bytes[0] === 0x89 && bytes[1] === 0x50 && bytes[2] === 0x4e && bytes[3] === 0x47) {
      const w = (bytes[16] << 24) | (bytes[17] << 16) | (bytes[18] << 8) | bytes[19];
      const h = (bytes[20] << 24) | (bytes[21] << 16) | (bytes[22] << 8) | bytes[23];
      if (w > 0 && h > 0) return { w, h };
    }
    // JPEG: сигнатура FF D8, ищем SOF-маркер с размерами
    ...
    console.warn("[Export] не удалось прочитать размеры картинки:", e);
  }
  return null;
}

// Размер под фиксированную высоту: возвращает [ширина_px, высота_px].
// heightCm — нужная высота в см. Ширина считается из пропорций картинки.
// Если пропорции неизвестны — используется fallbackRatio (ширина/высота).
function sizeByHeight(buf, heightCm, fallbackRatio = 1) {
  const PX_PER_CM = 37.795; // 96 DPI
  const targetH = Math.round(heightCm * PX_PER_CM);
  const dims = buf ? getImageDims(buf) : null;
  const ratio = dims && dims.h > 0 ? dims.w / dims.h : fallbackRatio;
...
// 🆕 ТЗ v2: водяной знак — текст ТТН/СМР в круге, не логотип
    ...
    const doc = new Docxtemplater(zip, {
      ...
      // Если в шаблоне нет какого-то {tag} - не падаем, а вставляем пустоту
      nullGetter: () => "",
    });

    const data = {
      // Логотип компании печатается над бланком во всех документах, СМР
      // включительно — так хочет заказчик. Его абзац стоит первым в
      // document.xml и сдвигает форму вниз на 0,7 см; место под него отыграно
      // в самом бланке (пустой абзац перед таблицей ужат до 2 pt) и размером
      // картинки — в СМР 85x28 px против 120x39 в остальных, см. getSize.
      // Убирать логотип из СМР пробовали — заказчик вернул.
      logo: companyLogo,
