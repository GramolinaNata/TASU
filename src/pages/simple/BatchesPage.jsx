...
export default function BatchesPage() {
  ...
  // 🆕 ТЗ v2: Табы Активные / Сформированные
  ...
  const openCreate = () => {
    setEditBatch(null);
    setForm({ ...EMPTY_FORM, number: "П" + String(Date.now()).slice(-6) });
    setShowForm(true);
  };
  ...
  // 🆕 ТЗ v2: Сформировать партию (перевод в "Сформированные")
  const handleForm = async (batch) => {
    ...
    try {
      // Сохраняем ведомость как HTML (можно потом распечатать в любой момент)
      let requestIds = [];
      try { requestIds = JSON.parse(batch.requestIds); } catch(e) {}
                  ...
                  <tr key={b.id} style={{
                    ...
                    <td>
                      <div style={{ display: "flex", gap: 6, flexWrap: 'wrap' }}>
                        {/* 🆕 ТЗ v2: Кнопка Сформировать / Вернуть */}
                        {b.isFormed ? (
                          <button
