...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .label {
    width: 100mm;
    /* ИСТОРИЯ ДВУХ БАГОВ — не трогать, не разобравшись.
       1) Было height: 150mm — ровно в лист. Рамка плюс округление мм→px
          иногда давали 150.0…мм с копейками, и хвост уезжал на второй лист.
       2) Потом стало max-height: 150mm — вторая страница ушла, но высота
          стала зависеть от содержимого, и при переполнении сжимался блок
          городов: город пропадал с наклейки.
       Сейчас высота ФИКСИРОВАННАЯ и на 1 мм короче листа. Миллиметр —
       это запас на округление: он гасит вторую страницу надёжнее, чем
       расчёт впритык. А фиксированная высота возвращает предсказуемость:
       излишек забирает QR-блок (flex ниже), а не блок городов. */
    height: 149mm;
    border: 1.5mm solid #000;
    display: flex;
    flex-direction: column;
  ...
  /* Смысловые блоки НЕ сжимаются. У flex-элементов flex-shrink по умолчанию 1:
     как только содержимое перерастало лист, они ужимались все разом, и первым
     схлопывался блок городов — самый крупный шрифт (8 мм). Наружу это выглядело
     как «город не напечатался». Теперь при нехватке места отдаёт только QR. */
  .header, .cities, .direction-row, .info-row, .num-row, .receiver-block {
    flex-shrink: 0;
  }
  ...
  /* Нижний блок для QR-кода — единственный, кто тянется и сжимается.
     Он забирает весь остаток фиксированных 149 мм, поэтому сумма блоков
     не «плавает» (один лист), а при нехватке места ужимается он, а не
     смысловые блоки выше. min-height: 0 обязателен: без него flex-элемент
     не может стать меньше своего содержимого и продавил бы лист. */
  .qr-block {
    flex: 1 1 auto;
    min-height: 0;
    display: flex;
  ...
  .qr-block img {
    /* Потолок и по блоку, и по ширине: в обычной ситуации код 30 мм,
       а если блок сжало — уменьшится вместе с ним, а не вылезет наружу. */
    max-height: 100%;
    max-width: 30mm;
    width: auto;
    height: auto;
...
<script>
  ...
  const handleExport = async (docTypeOverride = null, roleOverride = null) => {
    ...
    try {
      ...
      // Тип документа определяем ТАК ЖЕ, как проверка автотранспорта выше
      // (kind), — с запасным вариантом на колонку act.type. Раньше здесь
      // учитывались только docTypeOverride и act.docType: у накладной с
      // type='ttn', но пустым docType, exportType выходил undefined, экспорт
      // проваливался в docx и печатал СМР. Один способ определения на обе
      // ветки — и такой накладной больше не бывает.
      const exportType = docTypeOverride || act.docType || act.type;
      ...
      // Сравнение без учёта регистра: в колонке type встречается и 'ttn',
      // и 'TTN'. При строгом сравнении вариант в верхнем регистре уходил
      // в docx — то есть снова в СМР.
      if (String(exportType || "").toLowerCase() === "ttn" && !act.isWarehouse) {
