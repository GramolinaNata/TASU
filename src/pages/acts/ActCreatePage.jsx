...
// Нормализация названия города (убираем служебные суффиксы справочников тарифов)
...
export default function ActCreatePage() {
  ...
  // И для склада, и для обычной перевозки суммы лежат в одном и том же
  // массиве warehouseServices (проект не разделяет их на отдельные сущности).
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      // Фикс "лишняя буква Т": getCompanies() отдавал синхронный кэш,
      // который мог быть ещё пустым — allCompanies.find() не находил
      // компанию, и genNumber() уходил в дефолтный префикс "T".
      // loadCompanies() гарантированно подтягивает актуальный список.
  ...
  const findDirectTariff = useCallback(
    (cityClean) =>
      allTariffs.find((t) => {
        const category = getTariffCategory(t);
        return (
          (category === "legal" || category === "private") &&
          cleanCityName(t.city) === cityClean
        );
      }),
    [allTariffs]
  );

  const findRegionalTariff = useCallback(
    (cityClean, totalWeightForRegion) => {
      for (const t of allTariffs) {
        const category = getTariffCategory(t);
        if (category !== "legal" && category !== "private") continue;

        const wr = t.weightRanges || {};
        const regions = Array.isArray(wr._regionalDeliveries) ? wr._regionalDeliveries : [];
        const match = regions.find((r) => cleanCityName(r.region) === cityClean);
        if (!match) continue;

        let regionalExtra = 0;
        const sortedRanges = Array.isArray(match.ranges)
          ? [...match.ranges].sort((a, b) => (a.maxWeight || 0) - (b.maxWeight || 0))
          : [];

        let matchedRange = sortedRanges.find((range) => totalWeightForRegion <= (range.maxWeight || 0));
        if (!matchedRange && sortedRanges.length > 0) {
          matchedRange = sortedRanges[sortedRanges.length - 1];
        }
        if (matchedRange) {
          regionalExtra = toNum(matchedRange.extra);
        }

        return { tariff: t, regionalExtra, regionLabel: match.region || route.toCity };
      }
      return null;
    },
    [allTariffs, route.toCity]
  );

  const calculateByTariff = useCallback(() => {
    if (!route.toCity) {
      alert("Сначала выберите город получателя");
      return;
    }

    const cityClean = cleanCityName(route.toCity);
    let tariff = findDirectTariff(cityClean);
    let regionalExtra = 0;
    let regionalDescription = "";

    if (!tariff) {
      const regional = findRegionalTariff(cityClean, cargoWeightKg);
      if (regional) {
    ...
    if (!tariff) {
      alert(
        `Тариф для направления "${route.toCity}" не найден. Добавьте его в Тарифы (или укажите регион в тарифе ближайшего города).`
      );
      return;
    }

    const wr = tariff.weightRanges || {};
    const isPrivateTariff = getTariffCategory(tariff) === "private";
    const r10 = toNum(wr.r10);
    const r20 = toNum(wr.r20);
    const r30 = toNum(wr.r30);
    const overKg = toNum(wr._pricePerKgOver20) || toNum(tariff.pricePerKgOver20);
    const pricePerCubic = toNum(wr._pricePerCubic) || toNum(tariff.pricePerCubic);

    let sum = 0;
    let description = "";

    if (calcByCubic) {
      const vol = toNum(cubicVolume) || cargoVolumeM3;
      if (vol <= 0) {
        alert("Укажите объём в м³");
        return;
      }
      if (pricePerCubic <= 0) {
        alert("Для этого направления не задан тариф КУБ");
        return;
      }
      sum = vol * pricePerCubic;
      description = `Доставка ${route.toCity} (${vol} м³ × ${pricePerCubic.toLocaleString()} тг/м³)`;
    } else {
      if (cargoWeightKg <= 0 && cargoVolumeM3 <= 0 && !cubicVolume) {
        alert("Укажите вес или объём груза");
        return;
      }
      if (overKg <= 0 && cargoWeightKg > 20) {
        alert("Для этого направления не задан тариф за кг.");
        return;
      }
      ...
      if (isPrivateTariff) {
        ...
        } else if (cargoWeightKg <= 20) {
          sum = r20;
          description = `Доставка ${route.toCity} (до 20 кг)`;
        } else if (cargoWeightKg <= 30) {
          sum = r30;
          description = `Доставка ${route.toCity} (до 30 кг)`;
        } else {
          sum = cargoWeightKg * overKg;
          description = `Доставка ${route.toCity} (${cargoWeightKg} кг × ${overKg.toLocaleString()} тг/кг)`;
        }
      } else if (cargoWeightKg <= 10) {
        sum = r10 || cargoWeightKg * overKg;
        description = `Доставка ${route.toCity} (до 10 кг)`;
      } else if (cargoWeightKg <= 20) {
        sum = r20 || cargoWeightKg * overKg;
        description = `Доставка ${route.toCity} (до 20 кг)`;
      } else {
        sum = cargoWeightKg * overKg;
        description = `Доставка ${route.toCity} (${cargoWeightKg} кг × ${overKg.toLocaleString()} тг/кг)`;
      ...
      const totalVol = toNum(cubicVolume) || cargoVolumeM3;
      if (totalVol > 0 && pricePerCubic > 0) {
        const sumByCube = totalVol * pricePerCubic;
        if (sumByCube > sum) {
          sum = sumByCube;
          description = `Доставка ${route.toCity} (${totalVol} м³ × ${pricePerCubic.toLocaleString()} тг/м³) [макс из 2]`;
        } else {
    ...
    const extraSum = toNum(tariff.extraSum);
    if (isPrivateTariff && extraSum > 0) {
      sum += extraSum;
      description += ` + доплата ${extraSum.toLocaleString()} тг`;
    }
  ...
  }, [route.toCity, calcByCubic, cubicVolume, cargoWeightKg, cargoVolumeM3, findDirectTariff, findRegionalTariff]);

  const addLoadersService = useCallback(() => {
    if (!route.toCity) {
      alert("Сначала выберите город получателя");
      return;
    }
    const count = parseInt(loadersCount, 10) || 0;
    if (count <= 0) {
      alert("Укажите количество грузчиков");
      return;
    }
    ...
    let pricePerLoader;
    if (cargoWeightKg <= 10) {
      pricePerLoader = toNum(wr.r10);
    } else if (cargoWeightKg <= 20) {
      pricePerLoader = toNum(wr.r20);
    } else {
      pricePerLoader = toNum(wr.r30);
    }
    ...
    setWarehouseServices((prev) => {
      const filtered = prev.filter((s) => (s.name || s.price) && !String(s.name).startsWith("Грузчики "));
  ...
  }, [route.toCity, loadersCount, allTariffs, cargoWeightKg]);
        ...
        {showRouteCard && (
          <div className="card_body">
            <div className="form_grid">
              <div className="field">
                <div className="label">Страна, город отправителя</div>
                <input value={route.fromCity} onChange={(e) => setRoute({ ...route, fromCity: e.target.value })} placeholder="Алматы" />
              </div>
              <div className="field">
                <div className="label">Страна, город получателя</div>
                <input value={route.toCity} onChange={(e) => setRoute({ ...route, toCity: e.target.value })} placeholder="Астана" />
              </div>
              <div className="field">
                <div className="label">Адрес отправителя</div>
