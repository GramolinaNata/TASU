...
function getSortValue(c, field) {
  switch (field) {
    ...
    case 'bin':          return (c.bin || '').toString().toLowerCase();
    case 'phone':        return (c.phone || c.email || '').toString().toLowerCase();
    case 'contactPhone': return (c.contactPhone || '').toString().toLowerCase();
    case 'bank':         return (c.bank || '').toString().toLowerCase();
    default:             return '';
  }
}
...
export default function CounterpartiesPage() {
  ...
  const [formData, setFormData] = useState({
    name: "", phone: "", contactPhone: "", email: "", companyName: "", bin: "", address: "",
    bank: "", bik: "", account: "", kbe: "", director: ""
  });
  ...
  const handleNew = () => {
    setEditingCp(null);
    setFormData({
      name: "", phone: "", contactPhone: "", email: "", companyName: "",
      bin: "", address: "", bank: "", bik: "", account: "", kbe: "", director: ""
    });
    setShowModal(true);
  };
  ...
  return (
    <>
      ...
      <div className="filter" style={{ marginTop: 20 }}>
        <div className="field" style={{ flex: 1 }}>
          <div className="label">Поиск</div>
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="Поиск по имени, БИН, названию или телефону..." />
        </div>
      </div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <SortableTh field="bin">БИН</SortableTh>
                <SortableTh field="phone">Телефон / Email</SortableTh>
                <SortableTh field="contactPhone">📞 Контактный №</SortableTh>
                <SortableTh field="bank">Банк</SortableTh>
                <th style={{ width: 120 }}>Действия</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={8} className="muted">Контрагенты не найдены.</td></tr>
              ) : filtered.map(c => (
                <tr key={c.id}>
                  <td><strong>{c.name}</strong></td>
                  <td>{c.director || "—"}</td>
                  ...
                  <td>
                    {c.contactPhone ? (
                      <span style={{ fontWeight: 600, color: '#1890ff' }}>{c.contactPhone}</span>
                    ) : "—"}
                  </td>
                  <td style={{ fontSize: '0.8rem' }}>
                    {c.bank ? `${c.bank} (БИК: ${c.bik || ''})` : "—"}
      ...
      {showModal && (
        <Modal title={editingCp ? "Редактировать контрагента" : "Новый контрагент"} onClose={() => setShowModal(false)}>
          <form onSubmit={handleSave} className="form_grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            {/* 🆕 ТЗ v2: Контактный номер */}
            <div className="field" style={{ gridColumn: 'span 2', background: '#fff7e6', padding: 12, borderRadius: 8, border: '1px solid #ffd591' }}>
              <div className="label">📞 Контактный номер (дополнительный)</div>
              <input
              ...
              <div style={{ fontSize: '0.75rem', color: '#666', marginTop: 4 }}>
                Дополнительный номер для связи (например, кладовщик, водитель и т.п.)
              </div>
            </div>
