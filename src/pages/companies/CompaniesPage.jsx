...
export default function CompaniesPage() {
  ...
  const openEdit = (c) => {
    setEditId(c.id);
    setForm({
      name: c.name, bin: c.bin, address: c.address, factAddress: c.factAddress || "",
      phone: c.phone || "", director: c.director || "", email: c.email || "",
      bank: c.bank || "", bik: c.bik || "", account: c.account || "",
      kbe: c.kbe || "", bankDetails: c.bankDetails || "",
      managerDetails: c.managerDetails || "", logo: c.logo || "",
    });
    setModalOpen(true);
  };
                ...
                sortedList.map((c) => (
                  <tr key={c.id} className={c.id === selectedId ? "row_selected" : ""}>
                    ...
                    <td style={{ textAlign: "right", display: "flex", justifyContent: "flex-end", gap: 8 }}>
                      ...
                      <button className="btn btn--sm btn--danger" onClick={() => onDelete(c.id, c.name)}>
                        Удалить
                      </button>
                    </td>
                  </tr>
      ...
      {modalOpen && (
        ...
        >
          <div className="form_grid" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div className="field">
              <div className="label">Общие реквизиты (Доп. информация)</div>
              <textarea className="input" style={{ minHeight: '60px', padding: '8px' }} value={form.bankDetails} onChange={(e) => setForm({ ...form, bankDetails: e.target.value })} placeholder="Прочие данные для документов" />
            </div>
            <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 10 }}>
