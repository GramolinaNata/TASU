...
export default function CarriersPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await api.carriers.list();
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Удалить перевозчика?")) {
      try {
        await api.carriers.delete(id);
        loadData();
      } catch (e) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editing) {
        await api.carriers.update(editing.id, formData);
      } else {
        await api.carriers.create(formData);
      }
      setShowModal(false);
      loadData();
    } catch (err) {
  ...
  return (
    <>
      <div className="navbar">
        ...
        <button className="btn btn--accent" onClick={handleNew}>+ Добавить перевозчика</button>
      </div>
      ...
      <div className="table_wrap" style={{ marginTop: 16 }}>
        {loading ? <Loader /> : (
          <table>
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={4} className="muted">Перевозчики не найдены.</td></tr>
              ) : filtered.map(c => (
                <tr key={c.id}>
      ...
      {showModal && (
        <Modal title={editing ? "Редактировать перевозчика" : "Новый перевозчик"} onClose={() => setShowModal(false)}>
          <form onSubmit={handleSave} className="form_grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div className="field" style={{ gridColumn: 'span 2' }}>
              <div className="label">Наименование / ФИО <span className="text_danger">*</span></div>
