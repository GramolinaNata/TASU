...
export default function ContractCreatePage() {
  ...
  const [formData, setFormData] = useState({
    type: "warehouse",
    role: "expeditor", // ТЗ: роль в транспортном договоре — expeditor | carrier
    number: "",
    date: new Date().toISOString().split('T')[0],
    actId: "",
    counterpartyId: "",
          ...
          <div className="card_body">
            <div className="form_grid">
              ...
              {formData.type === 'transport' && (
                <div className="field field--full">
                  <div className="label">Наша роль в договоре</div>
                  <div style={{ display: 'flex', gap: 20, marginTop: 10 }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                      <input type="radio" name="role" value="expeditor" checked={formData.role === 'expeditor'} onChange={e => setFormData({ ...formData, role: e.target.value })} style={{ height: 'auto' }} />
                      <span>Экспедитор</span>
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                      <input type="radio" name="role" value="carrier" checked={formData.role === 'carrier'} onChange={e => setFormData({ ...formData, role: e.target.value })} style={{ height: 'auto' }} />
