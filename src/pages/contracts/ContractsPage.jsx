...
//     if (s) {
//       list = list.filter((c) => {
//         const hay = [c.number, c.customerName].filter(Boolean).join(" ").toLowerCase();
//         return hay.includes(s);
//       });
//     }
//     return list;
...
//         <div className="field" style={{ minWidth: 200, flex: 1 }}>
//           <div className="label">Поиск</div>
//           <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Номер договора, заказчик..." />
//         </div>
//         <div className="field" style={{ width: 200 }}>
//           <div className="label">Тип договора</div>
//           <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)}>
...
//                             </Link>
//                             {c.status !== 'canceled' && (
//                               <button className="actions-item" onClick={() => handleExport(c)} disabled={exportLoading} style={{ opacity: exportLoading ? 0.7 : 1 }}>
//                                 <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
//                                 {exportLoading ? "Загрузка..." : "Экспорт (.docx)"}
//                               </button>
//                             )}
...
function getSortValue(c, field) {
  switch (field) {
    case 'number':       return (c.number || '').toString().toLowerCase();
    case 'date':         return new Date(c.date || c.createdAt || 0).getTime();
    case 'status':       return (c.status || '').toString().toLowerCase();
    case 'type':         return (c.type || '').toString().toLowerCase();
    case 'customerName': return (c.customerName || '').toString().toLowerCase();
    default:             return '';
  }
}
...
export default function ContractsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return sorted;
  }, [contracts, q, company, typeFilter, sortBy, sortOrder]);
          ...
          <table className="table_fixed">
            <thead>
              <tr>
                ...
                <SortableTh field="type">Тип</SortableTh>
                <SortableTh field="customerName">Заказчик</SortableTh>
                {(!isAccountant || isAdmin) && <th style={{ width: 220, textAlign: "right" }}>Действия</th>}
              </tr>
            </thead>
            <tbody>
